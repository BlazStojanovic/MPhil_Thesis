...
The optimal control problem for the ground state in continuous state space

Holland~\cite{holland1977cost} arrives by means of exponential transform of the wave function $\psi = \exp(-U(x))$
\begin{equation}
...
\end{equation}
The control problem is solved for drift 
\begin{equation}
	v = \frac{\nabla \psi}{\psi} = -\nabla U
\end{equation}

\begin{equation}
C[v]=\int d x\left[\frac{1}{2}\left|v\left(X_{t}\right)\right|^{2}+V\left(X_{t}\right)\right] \pi(x \mid v) .
\end{equation}
\begin{equation}
\frac{1}{2} \nabla^{2} \pi-\nabla \cdot(v \pi)=0
\end{equation}
\begin{equation}
C[v]=\int d x\left[\frac{1}{2}(\nabla \psi)^{2}+V\left(X_{t}\right) \psi^{2}\right]
\end{equation}
